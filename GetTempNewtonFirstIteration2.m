function [GuessTc,I2R,I2Rprime,Prad,PradPrime,PradPrimePrime,Pcon,PconPrime,PconPrimePrime] =GetTempNewtonFirstIteration2(I,Ta,H,D,phi,Vw,alpha,beta,epsilons,Psol,GuessTc,fGrPr,fReNu,fNuRe)
    ...
    for i=1:row
        if(GuessTc(i)>Ta)
            Gr(i)=(g*(D^3).*(GuessTc(i)-Ta))./(Tfilmk(i).*(vf(i).^2));   
            ...
            GrPrimePrime(i)=(g*(D^3)).*(((Tfilmk(i)^2).*(vf(i)^3)).*dtop-...
                ...
                ((Tfilmk(i)^4).*(vf(i)^6));
        elseif(GuessTc(i)<Ta)
            Gr(i)=(g*(D^3).*(Ta-GuessTc(i)))./(Tfilmk(i).*(vf(i).^2));  
